...
def format_input(puzzle_input):
    ''' return list of lists from puzzle_input.
        puzzle_input: string separated by new lines.
    '''
    return [list(map(int, i.strip().split('\t'))) for i in puzzle_input.split('\n')]

def solution(formatted_data):
    ''' solution to p2
        formatted_data: list of lists
    '''
    answer = 0
    for sublist in formatted_data:
        answer += (max(sublist) - min(sublist))
    return answer
