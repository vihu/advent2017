...
def solver(input_str):
    ''' test if passphrase is valid. return True or False
    '''
    return len(set(input_str.split(" "))) == len(input_str.split(" "))

def solution(input_str):
    input_list = [''.join(sorted(i)) for i in input_str.split(" ")]
...
if __name__ == '__main__':
    ...
    llist = [solution(i.strip()) for i in data.split("\n")]
    print(len([i for i in llist if i == True]))
